...
class Model(dict, metaclass=ModelMetaClass):
    ...
    async def remove(self):
        ...
        if rows != 1:
            logging.warning('failed to remove by primary key: affected rows:' % rows)
...
async def test_save(loop):
    ...
    await create_pool(loop, user='root', password='000000', db='awesome')
    user = Test(id=0, name='c', password='123', )
    print('why?')
    await user.save()
    print('ok')
...
async def main():
    await asyncio.wait([test_save(loop), test_findAll(loop)])
